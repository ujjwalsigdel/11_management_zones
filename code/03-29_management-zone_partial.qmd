...
# Data import  
```{r all_v}
all_v <- read_sf("../../10_stability_validation/data/all_v.geojson")

all_v
```

```{r boundary_w}
boundary_w <- read_sf("../../05_digitizing/output/boundary.shp") %>% 
...
zone_s <- zone_df %>% 
  ...
  # Applying focal filter
    focal2(w = matrix(1,5,5),
           fun = "mean",
           na.rm = T) %>%         #when we have odd number for matrix, it has the center cell eg. 3x3, 5x5....)
  # Transforming from raster back to vector
    st_as_sf() %>% 
  # Interpolating to fill to boundary
    gstat::gstat(formula = cluster ~ 1,
          data = .) %>% 
    predict(grid_r) %>% 
  # Transforming from raster back to vector
    st_as_sf() %>% 
